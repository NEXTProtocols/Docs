...
<Note>All the training is made with 10 pictures of `sfya_nosupir`.</Note>

<Tabs>
  <Tab title="Flux 1.0 Dev">

    ## FA2 and Accelerate training.
    - Training steps : `2000 steps`
    - Batch size : `1`

    <CodeGroup dropdown>

    ```toml config_lora.toml expandable lines wrap
    ...
    ```json config_lora.json expandable lines wrap
    {
        "LoRA_type": "Flux1",
        "LyCORIS_preset": "full",
  ...
  <Tab title="Flux 1.0 Krea">
    ...
    unet_lr = 5e-5
    vae_batch_size = 4
    wandb_run_name = "rtx5090-fluxkrea"
    xformers = true
    ```
